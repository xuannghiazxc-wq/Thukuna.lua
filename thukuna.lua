...
-- TAB HOME (HIỂN THỊ FPS)
local HomeTab = Window:CreateTab("Home", 4483362458)
local Label = HomeTab:CreateLabel("FPS: " .. math.floor(workspace:GetRealPhysicsFPS()))
spawn(function()
    while wait(1) do
        Label:Set("FPS: " .. math.floor(workspace:GetRealPhysicsFPS()))
    end
...
FarmTab:CreateToggle({
   Name = "Accept Farm (Bật để đánh)",
   CurrentValue = false,
   Callback = function(Value) _G.AcceptFarm = Value end,
})

-- LOGIC FARM CƠ BẢN (GIỮ KHOẢNG CÁCH)
spawn(function()
    while wait() do
        if _G.AcceptFarm then
            pcall(function()
...
-- CÁC TAB TRỐNG ĐỂ BẠN TỰ THÊM TÍNH NĂNG
Window:CreateTab("Sea Event")
Window:CreateTab("V4 & Race")
Window:CreateTab("PvP")
Window:CreateTab("Setting")

Rayfield:Notify({Title = "Thành Công", Content = "Thukuna Hub đã sẵn sàng!"})
